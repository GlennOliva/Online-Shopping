...
<form action="" method="post" class="box">
   ...
   <div class="flex">
   <div class="price"><span>₱</span><?= number_format($fetch_cart['price'], 2, '.', ','); ?><span></span></div>

      <input type="number" name="qty" class="qty" min="1" max="<?= $product_stock; ?>" onkeypress="if(this.value.length == 2) return false;" value="<?= $fetch_cart['quantity']; ?>">
      <button type="submit" class="fas fa-edit" name="update_qty"></button>
   </div>
   <div class="sub-total"> sub total : <span>₱<?= number_format($sub_total = ($fetch_cart['price'] * $fetch_cart['quantity']), 2, '.', ','); ?></span> </div>

   <div class="stock">Stock: <?= $product_stock <= 0 ? "Out of Stock" : $product_stock; ?></div>
...
?>
  ...
  <button type="submit" name="check" class="btn <?= ($grand_total > 0)?'':'disabled'; ?>">proceed to checkout</button>
   </form>
